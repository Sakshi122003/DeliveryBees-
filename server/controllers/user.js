...
exports.register = async (req, res) => {
    ...
    try{// check if user already exists
        ...
        transporter.sendMail({
            ...
            html:"<h1>welcome to Delivery Bees</h1>"
        });
        res.status(201).json({
            ...
            .then(user=>{
                ...
                user.save().then((result)=>{
                    transporter.sendMail({
                        to: user.email,
                        from: "deliverys589@gmail.com",
                        subject: "OTP to log into Delivery Bees",
                        html:
                        `<h4> Your OTP for logging into Delivery Bees is : ${enroute_otp}, <p>use this OTP to complete the verification process.</p> </h4>`
                    })
...
exports.resetpassword = (req, res) => {
    ...
    /*if (!req.body.userType || !req.body.email) {
        return res.status(400).json({ error: "UserType and email are required fields" });
    }*/
    crypto.randomBytes(32, (err, buffer)=> {
        ...
        console.log('Generated Reset Token:', token_rs);
        User.findOne({userType:req.body.userType,email:req.body.email})
        .then(user=>{
            if(!user){
                console.log('User not found:', req.body.userType, req.body.email);
                return res.status(422).json({
...
exports.newpassword = async (req, res) => {
    ...
    console.log('Received token:', sentToken);
...
exports.orderemail = async (req, res) => {
    ...
    to: email,
    from: "deliverys589@gmail.com",
    subject: "Delivery Bees Payment Invoice",
